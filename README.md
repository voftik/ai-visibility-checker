...
Web-режим использует принудительный OpenRouter web tool, а Perplexity — native Sonar search. Memory-режим явно выключает web plugin и запрещает tool calls. Одна только надпись `mode="memory"` ничего не доказывает: ответ войдёт в метрики только после проверки телеметрии.
