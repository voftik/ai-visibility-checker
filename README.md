...
Текущая версия разметки — `annotations-v16`. Она детерминированно распознаёт
...
область на следующем peer heading или владельце. Это изменение семантики
намеренно подняло версию с v15: при пересборке сохранённого run старые derived
аннотации переразмечаются, тогда как panel/raw-ответы моделей не запрашиваются
и не изменяются.
...
4. Отдельный Gemini round 3 заново проверяет результат. Полный raw каждого repaired `answer_id` включается в payload как mandatory evidence до стратифицированной выборки; если он отсутствует, усечён или не помещается в жёсткий бюджет, gate закрывается до вызова. Публикация разрешена только по полному валидному `pass` первичного ответа; compact/semantic repair, deterministic fallback и повышение `revise`/`block` до `pass` запрещены. Оборванный или неразбираемый round 3 всегда закрывает gate.
...
Перед каждым фактическим provider POST ячейка атомарно получает короткий
`running:<generation>:<token>` claim, а в `run_artifacts` создаётся отдельная
append-only reservation. Поэтому параллельные worker не оплачивают одну ячейку
дважды, поздний worker не может перезаписать новый результат, а 3 200- и
6 400-токенные попытки имеют разные audit artifacts с полным raw, request hash,
transport, citations, usage и ошибкой. При resume старый failed raw сначала
импортируется в неизменяемый artifact и только после этого ячейка может быть
пересчитана; completed raw не открывается повторно.
...
| `PIPELINE_ORCHESTRATOR_MAX_CALLS_PER_RUN` | `2` | Жёсткий лимит запросов к planner на одну проверку; внутри epoch transport retry отключён |
...
Recovery-orchestrator не участвует в штатном happy path. После исчерпания локальных repair-попыток он получает сжатый incident, факты, digest и короткий allowlist действий. Модель не исполняет код, не меняет raw-ответы, SQL или метрики. Решение проходит кодовую проверку области, сохраняется в durable-журнале `recovery_epochs`, исполняется фиксированным обработчиком и закрывается сравнением before/after digest. Durable scope связывает план с фактами, allowlist действий и разрешёнными answer/artifact id; при повторном использовании код заново проверяет scope и `plan_digest`. Raw-ответ planner и digest фактического входа сохраняются в `usage_json._aiv_orchestrator`. Один epoch делает не более одного provider request: повторы transport и response-contract ошибок для Fable отключены. Одинаковое действие для того же fingerprint второй раз запрещено. `acceptance_checks` — не свободный текст модели, а точный набор исполняемых кодом проверок текущего stage: лишний или отсутствующий код останавливает recovery fail-closed. Потерявший lease воркер не может сохранить или завершить recovery epoch; повторное исполнение одного плана также ограничено отдельным бюджетом.
...
На момент обновления этого README в suite 502 теста.
