...
class DurableQueueTests(unittest.IsolatedAsyncioTestCase):
        ...
        if self.run_ids:
            async with SessionLocal() as session:
                await session.execute(
                    delete(Run).where(Run.id.in_(self.run_ids))
                )
    ...
    async def test_expired_saved_only_lease_restores_terminal_state(self) -> None:
        ...
        self.assertIsNone(
            await _claim_next_run("generic-owner", lease_seconds=90)
        )
    ...
    async def test_terminal_failed_marker_restores_previous_metadata(
        ...
        last_written_report = {
            "narrative": {"headline": "Последняя атомарная запись"}
        }
        run_id = await self._add_run(
    ...
    async def test_preview_child_is_cancelled_when_crawl_setup_fails(self) -> None:
        ...
        with (
            patch.object(
                ...
                "discover_site_pages",
                new=AsyncMock(
                    return_value=[("https://example.com/", "home")]
                ),
            ),
            patch.object(
                crawler,
    ...
    async def test_sqlite_queue_pragmas_and_unique_guard_are_enabled(self) -> None:
        async with SessionLocal() as session:
            ...
            indexes = (
                await session.execute(text("PRAGMA index_list('runs')"))
            ).all()
    ...
    async def test_sse_snapshot_helpers_are_retry_epoch_aware(self) -> None:
        ...
        self.assertIsNone(
            _last_sequence_for_epoch("2:18", stream_epoch=3)
        )
        self.assertIsNone(
            _last_sequence_for_epoch("18", stream_epoch=3)
        )
