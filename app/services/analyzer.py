...
- UA-асимметрии (один AI-вендор пропускается, другой режется). Анализируй ВСЕ четыре провайдера: OpenAI, Anthropic, Perplexity, DeepSeek - не только три первых. Если DeepSeek был в прогоне и его поведение существенно отличается от других вендоров (в любую сторону) - это самостоятельная яркая находка
...
Важно про учёт AI-провайдеров. В пайплайне утилиты могут быть включены user-agents четырёх AI-провайдеров: OpenAI (GPTBot, OAI-SearchBot, ChatGPT-User), Anthropic (ClaudeBot, anthropic-ai, Claude-Web), Perplexity (PerplexityBot, Perplexity-User), DeepSeek (DeepSeekBot, DeepSeek-User). Не все могут быть включены в каждом конкретном прогоне - но если в данных прогона есть проба с user_agent_label из любой провайдерской группы, в отчёте должны быть сделаны выводы про этого провайдера наравне с остальными.

DeepSeek - китайский провайдер без официальной документации краулеров. Его упоминание в отчёте имеет особый статус: при описании поведения DeepSeek всегда добавляй короткую оговорку "наблюдения по UA-строке, не подтверждены официально DeepSeek". Это нужно один раз при первом существенном упоминании DeepSeek в отчёте, дальше можно не повторять.

Если в прогоне DeepSeek-агенты не были включены - просто не упоминай DeepSeek в отчёте вообще. Не пиши "DeepSeek в прогоне отсутствовал". Это бессодержательная фраза. Молчание здесь - правильное поведение.
...
- Какие AI-агенты блокируются (нет / OpenAI / Anthropic / Perplexity / DeepSeek / комбинация / все)
